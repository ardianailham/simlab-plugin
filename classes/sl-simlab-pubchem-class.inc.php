...
class SL_SIMLAB_PubChemClass
{
    ...
    /**
     * AJAX handler — accepts POST with action=sl_pubchem_lookup and name=<compound_name>
     */
    ...
    private static function _extract_ghs($node, &$statements, &$pictograms, &$signal_word)
    {
        ...
        if ($val && is_array($val)) {
                ...
                foreach ($swms as $swm) {
                    ...
                    foreach ($swm['Markup'] ?? [] as $markup) {
                        ...
                        if (preg_match('/GHS\d+/i', $extra . $url, $m)) {
                            $code = strtoupper($m[0]);
                            if (! in_array($code, $pictograms, true)) {
                                $pictograms[] = $code;
                            }
                        }
                    }
            ...
            if (stripos($name, 'Hazard Statement') !== false && stripos($name, 'Precautionary') === false) {
                foreach ($swms as $swm) {
                    $s = trim($swm['String'] ?? '');
                    if ($s && ! in_array($s, $statements, true)) {
                        $statements[] = $s;
                    }
                }
            }
    ...
    /**
     * Return the local plugin asset URL for a GHS pictogram code like "GHS02".
     * Falls back to a generic local asset if the code cannot be normalized.
     */
    private static function _ghs_icon_url($code)
    {
        $num = preg_replace('/[^0-9]/', '', $code);
        if (! $num) {
            return defined('SL_SIMLAB_PATH')
                ? SL_SIMLAB_PATH . 'assets/ghs/GHS01.svg'
                : plugin_dir_url(dirname(__DIR__)) . 'assets/ghs/GHS01.svg';
        }

        $file = 'GHS' . str_pad($num, 2, '0', STR_PAD_LEFT) . '.svg';
        return defined('SL_SIMLAB_PATH')
            ? SL_SIMLAB_PATH . 'assets/ghs/' . $file
            : plugin_dir_url(dirname(__DIR__)) . 'assets/ghs/' . $file;
    }
}
