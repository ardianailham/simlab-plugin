...
if (isset($_GET['hapus'])) {
  $id = intval($_GET['id']);
  check_admin_referer('sl_hapus_logbook_alat_' . $id);
      ...
      <div class="card shadow-sm">
        <div class="card-body">
          ...
          <table class="table table-sm border-0">
            ...
            <tr>
              <th>Peminjam</th>
              <td>: <?= esc_html(get_userdata($data['user_id'])->display_name); ?></td>
            </tr>
            <tr>
              <th>Jumlah Pinjam</th>
              <td>: <?= esc_html($data['qty']); ?> Unit</td>
            </tr>
            <tr>
              <th>Waktu Mulai</th>
              <td>: <?= esc_html(wp_date('d M Y H:i', (int) $data['start_date'])); ?></td>
            </tr>
            <tr>
              <th>Waktu Selesai</th>
              <td>: <?= esc_html(wp_date('d M Y H:i', (int) $data['end_date'])); ?></td>
            </tr>
            <tr>
              <th>Status Saat Ini</th>
              <td>: <span class="badge bg-info"><?= esc_html($data['name']); ?></span></td>
            </tr>
          </table>
          <div class="mt-4">
            <a href="?page=<?= esc_attr($obj->plugin_slug . $obj->menu_slug); ?>"
  ...
  foreach ($data as $booking) {
    ...
    $text_color = '#ffffff';
    if ($booking['name'] == 'Accepted' || $booking['name'] == 'Completed')
      $color = '#198754'; // Success
    if ($booking['name'] == 'Pending') {
      $color = '#ffc107';
      $text_color = '#000000';
    } // Warning
    if ($booking['name'] == 'Rejected')
      $color = '#dc3545'; // Danger
    if ($booking['name'] == 'Ongoing')
      $color = '#0dcaf0'; // Info

    $events[] = [
      'id' => $booking['id'],
      'title' => '[' . $booking['qty'] . '] ' . $booking['Nama_Alat'] . ' (' . $user_name . ')',
      'start' => wp_date('c', (int) $booking['start_date']),
      'end' => wp_date('c', (int) $booking['end_date']),
  ...
  <div class="d-flex justify-content-between align-items-center mb-4">
    ...
    ?>
    <a href="<?= esc_url($add_booking_url); ?>" class="btn btn-success shadow-sm"><i class="fa fa-plus me-1"></i> Tambah
      Booking</a>
  </div>
        ...
        <tbody>
          ...
          <?php foreach ($data as $booking): ?>
            <tr>
              ...
              <td><i class="fa fa-user-circle-o me-1 text-muted"></i>
                <?= esc_html(get_userdata($booking['user_id'])->display_name ?? 'Unknown'); ?></td>
              <td><?= esc_html($booking['qty']); ?></td>
              <td class="small">
                <div class="text-success"><i class="fa fa-arrow-right"></i>
                  <?= wp_date('d M Y H:i', (int) $booking['start_date']); ?></div>
                <div class="text-danger"><i class="fa fa-arrow-left"></i>
                  <?= wp_date('d M Y H:i', (int) $booking['end_date']); ?></div>
              </td>
              <td>
                <?php
              ...
              <td>
                <div class="d-flex justify-content-center gap-1">
                  ...
                  <?php if (SL_SIMLAB_Auth::can_delete_log($user_id, $booking['user_id'])) { ?>
                    <a href="<?= wp_nonce_url('?page=' . esc_attr($obj->plugin_slug . $obj->menu_slug) . '&hapus&id=' . intval($booking['id']), 'sl_hapus_logbook_alat_' . intval($booking['id'])); ?>"
                      class="btn btn-sm btn-outline-danger" onclick="return confirm('Hapus logbook ini?');" title="Hapus"><i
